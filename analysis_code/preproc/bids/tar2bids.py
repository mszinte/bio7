...
>> python bids/tar2bids.py [meso_proj_dir] [meso_home_sw_dir] [subject_number]
-----------------------------------------------------------------------------------------
"""
...
parser = argparse.ArgumentParser(
    description = 'Convert subjects .tar files in sourcedata to bids. Uses bids structure defined in bids_heuristic_file_{project}.py.',
    epilog = 'Notes: 1. Replace {user} and {project} in examples according to data location and project to run analysis on, OR, your username depending on input (see above). 2. Penny needs to update [meso_home_sw_dir] with altered organisation of codes: /home/{user}/projects/{project}/analysis_code/preproc/bids. 3. Currently untested OPTION 1. treating any untreated subjects:- all')
parser.add_argument('[meso_proj_dir]', help='path to data project directory on mesocentre. e.g., /scratch/{user}/data/{project}')
parser.add_argument('[meso_home_sw_dir]', help='path to YOUR home software directory on mesocentre. e.g., /home/{user}/softwares')
...
# possible solutions for removal/automation of sys.argv[2]
# os.getlogin() : option to explore, only works if running from mesocentre, not invibe, will return mesocentre userid e.g., ptilsley
# invibe: /home/ptilsley/disks/meso_H/projects/bio7 mesocentre: /home/ptilsley/projects/bio7

# Check inputs
# ------------
if os.path.isdir(meso_proj_dir):
    "meso project input OK"
else:
    "meso project input doesnt exist"
    exit()


if os.path.isdir(meso_home_sw_dir):
    "meso software input OK"
else:
    "meso software input doesnt exist"
    exit()
...
#----------------------
tar2bids_file = [sif for sif in os.listdir(meso_home_sw_dir) if "tar2bids" in sif and sif.endswith(".simg")]
tar2bids_path = os.path.join(meso_home_sw_dir, tar2bids_file[0])

# Finding .bidsignore file
#-------------------------
bidsignore_file = [bd for bd in os.listdir(meso_home_sw_dir) if "bidsignore" in bd]
bidsignore_path = os.path.join(meso_home_sw_dir, bidsignore_file[0])

# Finding heuristics file
#------------------------
heuristics_file = [hr for hr in os.listdir(meso_home_sw_dir) if "heuristic" in hr and meso_proj_dir.split("/")[-1] in hr]
heuristics_path = os.path.join(meso_home_sw_dir, heuristics_file[0])
...
if subject_level == "all":
    ...
    for subj in subtobids:
        ...
        for subsess in subj_tars:
            ...
            #---------------------------
            print(["singularity","run", "-e", "-B", meso_proj_dir + "," + meso_home_sw_dir, tar2bids_path, "-h", heuristics_path, \
                   "-T", inp, "-w", meso_dir_bidswd, "-b", bidsignore_path, "-o", meso_proj_dir, subsess_tarpath])            
            
            subprocess.call(["singularity","run", "-e", "-B", meso_proj_dir + "," + meso_home_sw_dir, tar2bids_path, "-h", heuristics_path, \
                             "-T", inp, "-w", meso_dir_bidswd, "-b", bidsignore_path, "-o", meso_proj_dir, subsess_tarpath])
