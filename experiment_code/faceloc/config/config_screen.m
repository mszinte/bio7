...
if stim.loc_number == 2
    
    stim.loc_left = [(screen.centerX-stim.OffsetX) (screen.centerY)];    
    stim.loc_right = [(screen.centerX+stim.OffsetX) (screen.centerY)];
    
    % keep same order of locations throughout following
    stim.loc_name = {'left' 'right'};
    stim.loc_coord = [stim.loc_left; stim.loc_right];

else  
    error_locs = ('desired number of stimuli locations not programmed'); 
end
