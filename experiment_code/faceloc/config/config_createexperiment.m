...
experiment2_task_design = table(trial_number,...
    trial_time_secs,...
    trial_time_frames,...
    cum_trial_time_secs,...
...
%sort experiment based on trial number to randomise
experiment2_task_design = sortrows(experiment2_task_design,'trial_number');
%then sort back based on target emotion reference.. 
experiment2_task_design = sortrows(experiment2_task_design,'target_emotion_ref');

%add run number block number and cumulative experiment time 
cum_trial_time_secs((1:task.trials),:) = cumsum(experiment2_task_design.trial_time_secs);
cum_trial_time_frames((1:task.trials),:) = cumsum(experiment2_task_design.trial_time_frames);
block_number((1:task.trials),:) = design.blocknumber;
run_number((1:task.trials),:) = task.run_number; 

%replace trial number with organised list 1:trial number total
experiment2_task_design.trial_number = design.sort_id;

%put in ordered block and run numbers and calculate cum trial times
experiment2_task_design.block_number = block_number; 
experiment2_task_design.run_number = run_number; 
experiment2_task_design.cum_trial_time_secs = cum_trial_time_secs; 
experiment2_task_design.cum_trial_time_frames = cum_trial_time_frames; 

%% Logging any errors into text file

log_errors = who("-regexp", '^error', '^warning');

z = fopen('experiment2_design_error_log.txt', 'w');
