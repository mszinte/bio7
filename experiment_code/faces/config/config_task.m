...
if mod(task.trialspemocat,time.soa_number) ~= 0
    
error_soas1 = ('number of soas not equal across categories')

task.trialspemocatpsoa = task.trialspemocat / time.soa_number;
...
stim.images_maxpemocat = min(minimages);
 ...
 stim.imagespsoa = stim.images_numberpemocat / time.soa_number;  
 
 if  round(stim.imagespsoa) == stim.imagespsoa
     % images per soa is integer 
 else
     warning_imagessoa = ('number of images for each soa doesnt divide, may be problematic to design')
 end
 ...
 task.trialspsoagroup = stim.images_numberpemocat; 
 task.trialspsoa = (task.trialspsoagroup / time.soa_number) * task.trialspimage;
 
 %% Creating plan task based on categories 
