...
for b = b_start:b_end
    ...
    Screen('DrawText',screen.main, 'REPONDEZ', screen.centerX-200,screen.centerY-60);
    Screen('DrawText',screen.main, 'La Joie ou La Peur?', screen.centerX-350,screen.centerY-00);
    Screen('DrawText',screen.main, '(Gauche)  (Droite)', screen.centerX-350,screen.centerY+60);
    Screen('Flip',screen.main);
    WaitSecs(5);
    ...
    for t = t_start:t_end
        ...
        while soa_tic < (experiment1_task_design.soa_secs(t) - screen.frame_duration_mfitest)
            Screen('DrawTexture',screen.main, target_texture, [],...
                [experiment1_task_design.location_x(t)-image_size_X...
                experiment1_task_design.location_y(t)-image_size_Y...
                experiment1_task_design.location_x(t)+image_size_X...
                experiment1_task_design.location_y(t)+image_size_Y]);
            Screen('DrawText',screen.main, '+', screen.centerX-25,screen.centerY-10);
    ...
    data.onset_block_rest(b) = GetSecs;
    
    %% Preseting cue for resting block task, 10 seconds 
    
    % Putting cue on screen for 5 seconds 
    Screen('DrawText',screen.main, 'REGARDEZ', screen.centerX-50,screen.centerY-20);
    Screen('DrawText',screen.main, 'Ne Repondez Pas', screen.centerX-300,screen.centerY+20);
    Screen('Flip',screen.main);
    WaitSecs(5);
    % Changing to fixation cross for 5 seconds 
    Screen('DrawText',screen.main, '+', screen.centerX-25,screen.centerY-10);
    ...
    for r = r_start:r_end
        ...
        % Drawing target texture screen
        % to delete if code OK - target_image = imread(stim.images.(cell2mat(experiment1_design.target_id(t)))(t).name);
        rest_image = imread(stim.images.(cell2mat(experiment1_rest_design.image_id(r)))(experiment1_rest_design.image_number_ref(r)).name);
        target_texture = Screen('MakeTexture',screen.main,rest_image); 
        
        image_size_original = size(rest_image); 
        image_size_X = round(image_size_original(2)*stim.ScalingFactor); 
        image_size_Y = round(image_size_original(1)*stim.ScalingFactor);
        
        % Recording which image selected - target
        data.rest_name_check(r) = cellstr(stim.images.(cell2mat(experiment1_rest_design.image_id(r)))(experiment1_rest_design.image_number_ref(r)).name);
           
        %% Presenting images 
        
        %% 1. Showing fixation cross for certain amount of jitter 
        data.rest_onset_trial(r) = GetSecs;
        ...
        while jitter_tic < (experiment1_rest_design.jitter_secs(r) - screen.frame_duration_mfitest)
            Screen('DrawText',screen.main, '+', screen.centerX-25,screen.centerY-10);
            jitter_flip = Screen('Flip',screen.main);
            jitter_tic = jitter_flip-data.rest_onset_trial(r);
        end
        
        %% 2. Showing target for different times (soa) 
        data.rest_onset_target(r) = GetSecs; 
        
        soa_tic = 0;
        
        while soa_tic < (experiment1_rest_design.soa_secs(r) - screen.frame_duration_mfitest)
            Screen('DrawTexture',screen.main, target_texture, [],...
                [experiment1_rest_design.location_x(r)-image_size_X...
                experiment1_rest_design.location_y(r)-image_size_Y...
                experiment1_rest_design.location_x(r)+image_size_X...
                experiment1_rest_design.location_y(r)+image_size_Y]);
            Screen('DrawText',screen.main, '+', screen.centerX-25,screen.centerY-10);
            soa_flip = Screen('Flip',screen.main); 
            soa_tic = soa_flip-data.rest_onset_target(r);
        end
        
%         %% 3. Showing mask for fixed amount of time 
%         
%         data.rest_onset_mask(r) = GetSecs;
%         
%         mask_tic = 0; 
%         
...
%             Screen('DrawText',screen.main, '+', screen.centerX-25,screen.centerY-10);
%             mask_flip = Screen('Flip',screen.main);  
%             mask_tic = mask_flip - data.rest_onset_mask(r);
%         end
         
        %% Showing fixation cross for fixed response time 

        data.rest_onset_response(r) = GetSecs;
        response_tic = 0;
        ...
        while response_tic < (experiment1_rest_design.response_secs(r) - screen.frame_duration_mfitest)
            ...
            Screen('DrawText',screen.main, '+', screen.centerX-25,screen.centerY-10);
            response_flip = Screen('Flip',screen.main);    
            response_tic = response_flip-data.rest_onset_response(r);
        end
